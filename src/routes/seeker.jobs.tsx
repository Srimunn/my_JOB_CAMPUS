...
function AvailableJobs() {
  const { data = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("jobs").select("id,title,company,location,job_type,category,created_at").order("created_at", { ascending: false });
  ...
  return (
    <DashboardShell title="Available Jobs" nav={seekerNav} requireRole="jobseeker">
      {isLoading ? <div className="text-muted-foreground">Loading…</div> : data.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-border bg-card p-10 text-center text-muted-foreground">No jobs available right now.</div>
      ) : (
