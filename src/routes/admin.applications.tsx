              ...
              return (
                <div key={a.id} className="flex flex-wrap items-start justify-between gap-3 p-4">
                  <div>
                    ...
                    <div className="text-sm text-muted-foreground">{profile?.email} · {profile?.phone}</div>
                    <div className="mt-1 text-sm">
                      Applied to {job ? <Link to="/job/$id" params={{ id: job.id }} className="text-primary underline">{job.title}</Link> : "—"} @ {job?.company}
                    </div>
                    <div className="mt-1 text-xs text-muted-foreground">{new Date(a.created_at).toLocaleString()} · {a.status}</div>
                  </div>
                  {a.resume_url && <Button size="sm" variant="outline" onClick={() => viewResume(a.resume_url!)}>View resume</Button>}
                </div>
              );
            })}
          </div>
