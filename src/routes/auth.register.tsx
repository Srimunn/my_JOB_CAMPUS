...
export const Route = createFileRoute("/auth/register")({
  head: () => ({ meta: [{ title: "Register — My Job Campus" }, { name: "description", content: "Create a free job seeker account on My Job Campus." }] }),
  component: Register,
});
        ...
        <div className="rounded-2xl border border-border bg-card p-8 shadow-sm">
          ...
          <form
            ...
            onSubmit={async (e) => {
              ...
              setLoading(true);
              const redirectTo = typeof window !== "undefined" ? `${window.location.origin}/seeker` : undefined;
              ...
              if (resume && data.user) {
                ...
                if (!upErr) {
                  await supabase.from("profiles").update({ resume_url: path }).eq("id", data.user.id);
                }
              }
              setLoading(false);
          ...
          >
            ...
            <Button type="submit" disabled={loading} className="w-full rounded-full">{loading ? "Creating…" : "Register"}</Button>
          </form>
...
function Field({ label, value, onChange, type = "text" }: { label: string; value: string; onChange: (v: string) => void; type?: string }) {
  return (
    <div>
      ...
      <input required type={type} value={value} onChange={(e) => onChange(e.target.value)} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm" />
    </div>
  );
}