...
export default function EditArticle() {
  ...
  // Populate values when article data is fetched
  useEffect(() => {
    if (article) {
      setTitle(article.title || "");
      setCategory(article.category || "Resume Building");
      setAuthorName(article.author_name || "My Job Campus Editor");
  ...
  if (isLoading) {
    return (
      <DashboardShell title="Edit Article" nav={adminNav} requireRole="admin">
  ...
  if (!article) {
    return (
      <DashboardShell title="Edit Article" nav={adminNav} requireRole="admin">
        <div className="text-center p-12 text-muted-foreground">Article details not found.</div>
      </DashboardShell>
    );
  }
        ...
        <form onSubmit={handleSubmit} className="space-y-6 rounded-2xl border border-border bg-card p-6 shadow-sm">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Write the long-form SEO article content here..."
