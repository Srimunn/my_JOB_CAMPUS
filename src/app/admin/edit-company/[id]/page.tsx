...
import { Building2, Upload, ChevronDown, ChevronUp } from "lucide-react";
...
export default function EditCompany() {
  ...
  // Load values when company data is fetched
  useEffect(() => {
    if (company) {
      setName(company.name || "");
      setDescription(company.description || "");
      setIndustry(company.industry || "Software & Tech");
      setLocation(company.location || "Bengaluru");
      setWebsiteUrl(company.website_url || "");
      setLogoUrl(company.logo_url || "");
      setFeatured(company.featured || false);
      setFocusKeyword(company.focus_keyword || "");
      setSeoTitle(company.seo_title || "");
      setSeoDescription(company.seo_description || "");
      setSlug(company.slug || "");
      setCanonicalUrl(company.canonical_url || "");
      setOgImage(company.og_image || "");
    }
  }, [company]);
  ...
  if (isLoading) {
    return (
      <DashboardShell title="Edit Company" nav={adminNav} requireRole="admin">
    ...
    <DashboardShell title="Edit Company" nav={adminNav} requireRole="admin">
      <div className="mx-auto max-w-4xl space-y-6">
        <form onSubmit={handleSubmit} className="space-y-6 rounded-2xl border border-border bg-card p-6 shadow-sm">
          ...
          {/* SEO score card is placed before advanced settings */}
