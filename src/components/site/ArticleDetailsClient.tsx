...
function BlogIllustration({ category, title, featured_image }: { category: string; title: string; featured_image?: string | null }) {
  if (featured_image) {
    return (
      <div className="relative w-full h-full min-h-[200px] md:min-h-[300px] rounded-2xl overflow-hidden">
        <img
          src={featured_image}
          alt={title}
  ...
  if (isInterview) {
    return (
      <svg className="w-full h-full max-h-[340px] text-primary/80" viewBox="0 0 200 120" fill="none" xmlns="http://www.w3.org/2000/svg">
        <defs>
        ...
        <rect x="45" y="50" width="32" height="38" rx="8" fill="var(--color-primary)" fillOpacity="0.1" stroke="var(--color-primary)" strokeWidth="1.5" />
        <circle cx="61" cy="35" r="11" fill="var(--color-primary)" fillOpacity="0.15" stroke="var(--color-primary)" strokeWidth="1.5" />
        <rect x="123" y="50" width="32" height="38" rx="8" fill="var(--color-accent)" fillOpacity="0.1" stroke="var(--color-accent)" strokeWidth="1.5" />
        <circle cx="139" cy="35" r="11" fill="var(--color-accent)" fillOpacity="0.15" stroke="var(--color-accent)" strokeWidth="1.5" />
        <path d="M 80 40 C 80 32 90 28 100 28 C 110 28 120 32 120 40 C 120 45 115 48 110 50 L 112 55 L 102 50 C 90 50 80 45 80 40 Z" fill="var(--color-accent)" fillOpacity="0.15" stroke="var(--color-accent)" strokeWidth="1" />
  ...
  if (isJobs) {
    return (
      <svg className="w-full h-full max-h-[340px] text-primary/80" viewBox="0 0 200 120" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <rect x="60" y="40" width="80" height="50" rx="10" fill="var(--color-primary)" fillOpacity="0.1" stroke="var(--color-primary)" strokeWidth="2" />
        <path d="M 82 40 V 28 C 82 25 84 23 87 23 H 113 C 116 23 118 25 118 28 V 40" stroke="var(--color-primary)" strokeWidth="2" strokeLinecap="round" />
        <rect x="91" y="58" width="18" height="12" rx="3" fill="var(--color-card)" stroke="var(--color-primary)" strokeWidth="1.5" />
  ...
  if (isResume) {
    return (
      <svg className="w-full h-full max-h-[340px] text-primary/80" viewBox="0 0 200 120" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <rect x="65" y="20" width="70" height="85" rx="6" fill="var(--color-card)" stroke="var(--color-border)" strokeWidth="2" />
        <rect x="75" y="32" width="25" height="8" rx="2" fill="var(--color-primary)" fillOpacity="0.25" />
        <circle cx="120" cy="36" r="6" fill="var(--color-accent)" fillOpacity="0.3" />
        <line x1="75" y1="50" x2="125" y2="50" stroke="var(--color-border)" strokeWidth="2" strokeLinecap="round" />
        <line x1="75" y1="62" x2="120" y2="62" stroke="var(--color-border)" strokeWidth="2" strokeLinecap="round" />
        <line x1="75" y1="74" x2="125" y2="74" stroke="var(--color-border)" strokeWidth="2" strokeLinecap="round" />
        <line x1="75" y1="86" x2="105" y2="86" stroke="var(--color-accent)" strokeWidth="2" strokeLinecap="round" />
        <circle cx="145" cy="80" r="16" fill="var(--color-accent)" fillOpacity="0.15" stroke="var(--color-accent)" strokeWidth="1.5" />
        <path d="M 139 80 L 143 84 L 151 75" stroke="var(--color-accent)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    );
  }
  ...
  return (
    <svg className="w-full h-full max-h-[340px] text-primary/80" viewBox="0 0 200 120" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <circle cx="100" cy="50" r="24" fill="var(--color-card)" stroke="var(--color-primary)" strokeWidth="2" />
      <path d="M 88 70 H 112 M 92 76 H 108 M 96 82 H 104" stroke="var(--color-primary)" strokeWidth="2" strokeLinecap="round" />
      <line x1="100" y1="15" x2="100" y2="22" stroke="var(--color-accent)" strokeWidth="2.5" strokeLinecap="round" />
      <line x1="68" y1="32" x2="74" y2="38" stroke="var(--color-accent)" strokeWidth="2.5" strokeLinecap="round" />
      <line x1="132" y1="32" x2="126" y2="38" stroke="var(--color-accent)" strokeWidth="2.5" strokeLinecap="round" />
    </svg>
  );
}

export function ArticleDetailsClient({ article, relatedArticles }: ArticleDetailsClientProps) {
    ...
    <SiteLayout>
      ...
      <section className="mx-auto grid max-w-6xl gap-8 px-4 py-12 lg:grid-cols-[1fr_280px] lg:px-8">
        <div className="space-y-10">
          ...
          <div className="rounded-3xl border border-border bg-gradient-to-br from-secondary/60 to-[#f5ebe6]/30 p-4 sm:p-8 flex items-center justify-center relative overflow-hidden shadow-sm">
          ...
          <article className="prose prose-slate max-w-none text-foreground/90 text-justify">
            {article.content.map((paragraph, index) => {
              ...
              // Check if paragraph contains HTML structure
              const containsHtml = /<[a-z][\s\S]*>/i.test(paragraph);
              ...
              if (containsHtml) {
                return (
                  <div
                    ...
                    className="leading-relaxed mb-6 text-[15px] md:text-base text-muted-foreground text-justify"
                  />
                );
              }
          ...
          <div className="pt-8 border-t border-border/60 flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex gap-2.5 flex-wrap">
              <a
                ...
                className="h-9 px-4 rounded-full bg-whatsapp text-white hover:bg-whatsapp/90 text-xs font-bold flex items-center gap-1.5 shadow-sm transition-all cursor-pointer"
              >
                WhatsApp
              </a>
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="h-9 px-4 rounded-full bg-[#1da1f2] text-white hover:bg-[#1da1f2]/90 text-xs font-bold flex items-center gap-1.5 shadow-sm transition-all cursor-pointer"
              >
                Twitter
              </a>
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="h-9 px-4 rounded-full bg-[#0077b5] text-white hover:bg-[#0077b5]/90 text-xs font-bold flex items-center gap-1.5 shadow-sm transition-all cursor-pointer"
              >
                LinkedIn
              </a>
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="h-9 px-4 rounded-full bg-[#1877f2] text-white hover:bg-[#1877f2]/90 text-xs font-bold flex items-center gap-1.5 shadow-sm transition-all cursor-pointer"
              >
                Facebook
              </a>
            </div>
          ...
          <div className="pt-10 border-t border-border/60">
            ...
            <form onSubmit={handleCommentSubmit} className="space-y-4">
              <div>
                <label htmlFor="comment" className="block text-xs font-bold text-foreground/80 mb-2 uppercase tracking-wide">
              ...
              <div className="grid gap-4 md:grid-cols-3">
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      placeholder="Name"
                      className="w-full rounded-2xl border border-border bg-card py-2.5 pl-4 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all"
                    />
                    <Edit3 className="absolute right-3.5 top-3.5 h-3.5 w-3.5 text-muted-foreground/60" />
                  </div>
                </div>
                ...
                <div>
                  <label htmlFor="website" className="block text-xs font-bold text-foreground/80 mb-2 uppercase tracking-wide">
                  ...
                  <div className="relative">
                    <input
                      ...
                      placeholder="Website"
                      className="w-full rounded-2xl border border-border bg-card py-2.5 pl-4 pr-10 text-sm focus:outline-none"
                    />
              ...
              <div className="flex items-start gap-2 pt-2">
                ...
                />
                <label htmlFor="save-info" className="text-xs text-muted-foreground/90 leading-tight select-none cursor-pointer">
                  {t("career.saveInfo")}
                </label>
              </div>
              ...
              <button
                type="submit"
                className="mt-4 px-8 py-3 bg-accent text-accent-foreground rounded-full text-xs font-bold flex items-center gap-1.5 hover:bg-accent/90 shadow-sm transition-all cursor-pointer"
              >
        ...
        {/* Sidebar */}
        <div className="h-fit">
          <Suspense fallback={<div className="h-96 w-full bg-[#FAF6F0] rounded-[32px] animate-pulse" />}>
          ...
          <div className="mx-auto max-w-6xl px-4 lg:px-8">
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
              {relatedArticles.map((relPost) => (
                <article
                  key={relPost.slug}
                  className="group rounded-2xl border border-border/60 bg-card overflow-hidden shadow-soft hover:shadow-md hover:-translate-y-1 transition-all duration-300 flex flex-col h-full"
                >
                  <div className="h-28 bg-gradient-to-br from-secondary/50 to-[#f5ebe6]/20 p-3 flex items-center justify-center border-b border-border/40">
                    <div className="w-full h-full max-w-[120px] relative">
                      <BlogIllustration
                        category={relPost.categories[0]}
                        title={relPost.title}
                        featured_image={relPost.featured_image}
                      />
                    </div>
                  </div>
